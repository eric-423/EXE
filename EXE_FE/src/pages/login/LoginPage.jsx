...
const LoginPage = () => {
  ...
  const handleCustomerSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 400) {
        setPhoneError("Số điện thoại này đã được đăng ký. Vui lòng đăng nhập.");
        return;
      }
    } catch (error) {
  ...
  const handleOTPSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/verify-code/verify?phoneNumber=${phoneNumber}&code=${otpValue}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const checkUserExists = async (phone) => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/api/v1/users/check-phone`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleChange = (element, index, isFirstPassword) => {
    ...
    if (isFirstPassword) {
      setPassword1([
        ...password1.map((d, idx) => (idx === index ? element.value : d)),
      ]);
    } else {
      setPassword2([
        ...password2.map((d, idx) => (idx === index ? element.value : d)),
      ]);
    }
  };
